...
url: https://github.com/EricMarcon/JTE-22-105
github-repo: EricMarcon/JTE-22-105
lang: en-US
...
Plots <- read_csv2("data/plots.csv")
# Abundances of species in each plot
Abundances <- read_csv2("data/abundances.csv")
# Number of plots
...
It now consists of 243 plots of various sizes and shapes, distributed in various forest types, in 30 sites across French Guiana.
We took into account the `r guyadiv_plots` one-hectare plots of the network (figure \@ref(fig:GuyaDivMap)).
They are located in `r guyadiv_locations` sites, which provides fairly good coverage of the variability of the forest.
They contain 43081 trees among which 415 were removed from the analyses because they could not be assigned to a species or morphospecies.

(ref:GuyaDivMap) The GuyaDiv network of tropical forest plots, 1-ha plots only. All trees above 10 cm DBH are localized, measured and determined botanically at species level. Each circle represents a location where several forest plots were established: the circle size is proportional to the number of plots. Paracou, Piste de Saint-Elie and Nouragues large inventories locations are shown (Paracou contains no 1-ha GuyaDiv plot). The map background represents the tree cover, as of ESA Land Cover [@Zanaga2021].

```{r}
#| label: GuyaDivMap
#| fig.cap: "(ref:GuyaDivMap)"
#| include: TRUE
#| out.width: 120%
...
Plots %>%
  group_by(Location) %>%
  summarise(nb = n(), x = mean(as.numeric(X_UTM)), y = mean(Y_UTM)) %>%
  # Convert UTM to lat/long
  st_as_sf(coords = c("x", "y"), crs = "+proj=utm +zone=22") %>%
...
"data/stations.csv" %>% 
  read_csv2() %>% 
  # Convert UTM to lat/long
  st_as_sf(coords = c("Xutm", "Yutm"), crs = "+proj=utm +zone=22") %>%
  st_transform(crs = 4326) ->
  stations

# Land cover. Download only if necessary (file is 500 MB)
library("geodata")
library("terra")
if (!file.exists("data/gufCover_trees_30s.tif")) {
  # Download the forest land cover 
  landcover(var = "trees", path = "data") %>% 
    # Crop it to French guiana
    terra::crop(guf_terra, mask = TRUE) %>%
    writeRaster(filename = "data/gufCover_trees_30s.tif")
}
...
ggplot() +
  # Tree cover
  geom_spatraster(
    data = rast("data/gufCover_trees_30s.tif"),
    alpha = 0.9
  ) +
  scale_fill_gradientn(
    colours = terrain.colors(8),
    trans = 'reverse',
    na.value = NA
  ) +
  # Plots
  ...
  labs(size = "# Plots", fill = "Tree cover", x ="", y = "") +
  # Scale bar
...
Then, if it is present in $A_1$, the probability to find it in turn in half $A_1$, denoted $A_2$, is also $a$, and so on.
...
```{r}
#| label: FrenchGuiana
# Extrapolation area in square km
A0 <- 80000
```
...
The estimator of order $k$ includes $f_1, f_2, \dots, f_k$, the number of species observed up to $k$ times.
...
In each 100 by 100 km cell of the grid, all available data is aggregated to obtain an incidence dataset.
The Chao2 estimator (whose formula is identical to that of Chao1, with $n$ equal to the number of grid cells) is finally applied: it combines the number of species observed in only one or two cells to estimate the number of unobserved species.
...
The estimated number of species per square kilometer, $c$, is respectively `r c_paracou %>% round(0)`, `r c_PSE %>% round(0)` and `r c_NOU %>% round(0)` in Paracou, Piste de Saint-Elie and Nouragues.
...
The mean number of trees per ha in the Guyadiv 1-ha plots is `r round(n_ha_arithmetic)`.
...
Extra <- exp(
  EA50lm$coefficients[1]) * 
  cumprod(
    rep(
      exp(EA50lm$coefficients[2]), 
      n_species-round(length(abd_est) / 4) + 1
    )
  )
ggplot() +
  # Extrapolation
  geom_point(
    data = data.frame(
      x = round(length(abd_est) / 4) + seq_along(Extra), 
      y = Extra
    ), 
    mapping = aes(x = x, y = y),
    color = "red"
  ...
  # Observed species
  geom_point(
    data = data.frame(x = seq_along(abd_est), y = abd_est), 
    mapping = aes(x = x, y = y),
    color = "grey"
  ) +
  # First 75%
  geom_point(
    data = data.frame(x = seq_along(EA75), y = EA75), 
    mapping = aes(x = x, y = y),
  ) +
  scale_y_log10() +
  # Set X axis values
  scale_x_continuous(labels = rlang::as_function(~ .x + 1)) +
...
Figure \@ref(fig:harte2009sar) shows the species-area curves obtained by successive doubling of the areas.
...
At the regional scale, the mixture of local communities makes a new pattern emerge, namely the power law of @Arrhenius1921.
...
A long empirical controversy [@Connor1979] opposed Arrhenius and @Gleason1922, who argued that the number of species predicted by the power law was far too high and proposed $\mathrm{S}(A)=z\ln A + c_g$ rather than the equivalent of eq.
\@ref(eq:Arrhenius), i.e. $\ln \mathrm{S}(A)=z\ln A + c_a$ (where $c_g$, $c_a$ and $c_f$ below are constants).
...
Its follows a log-series distribution [@Hubbell2001] with Fisher's $\alpha$ equal to $\theta$, known as the fundamental biodiversity number.
...
Yet, our data fit a log-series distribution quite well (figure \@ref(fig:logseries)): empirical tests are not efficient to reject the model at the regional scale [@Connor1979].
...
So the wider the sampled area, the less distinguishable from the metacommunity the data will be, but at the scale of French Guiana, roughly 1% of Amazonia, many less species are present than in a sample of the same size taken across the whole metacommunity, even if we ignore environmental filtering.
...
Theoretically, it is far more robust than the mixture of abundance data: the local distribution of each plot is cancelled out by its transformation into incidence data.
...
## The number of species is around `r round(S/100)*100`

Finally, our estimations of the number of tree species in the 8-million-hectare forest of French Guiana is close to `r round(S/100)*100`, with a quite wide confidence interval due to the variability in the estimation of both the number of trees in a square kilometer and the power-law parameter.
...
The GuyaDiv and GuyaFor databases together currently list more than 300 of these unnamed species, but @Molino2022 selected only 143 of them for their checklist, the ones that were best characterized and best illustrated by good quality herbarium specimens.
...
Therefore, the estimate of `r S %>% round(0)` spp.
given here seems quite plausible, given the state of our knowledge.

A improvement perspective is the aggregation of all sources of localized data at the scale of French Guiana, including all Guyadiv plots, the Guyafor network [@Jaouen2021], herbarium collections and various scientific projects, to proceed with an incidence-based estimation of richness following @CazzollaGatti2022.
...
```{r}
#| label: hyperdominance
#| fig.cap: "(ref:hyperdominance)"
#| include: true
# Hyperdominance
data.frame(
      x = seq_along(abd_est), 
      y = cumsum(as.numeric(abd_est))
    ) %>% 
  ggplot() +
    # Extrapolation
    geom_point(aes(x = x, y = y)) +
    geom_hline(yintercept = n_trees / 2, lty = 2, col = "red") +
    geom_vline(xintercept = hdSpecies, lty = 2, col = "red") +
    # Set X axis values
    scale_x_continuous(labels = rlang::as_function(~ .x + 1)) +
    labs(x = "Rank", y = "Cumulated Number of Trees")
```
